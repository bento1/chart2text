...
from minigpt4.datasets.datasets.chart2text_warmup import SummaryChartDataset
...
from minigpt4.common.eval_utils import prepare_texts, init_model, eval_parser,init_model_with_text_processor
...
model, vis_processor, text_processor = init_model_with_text_processor(args)
...
if 'chart2text_warmup' in args.dataset:

    batch_size = cfg.evaluation_datasets_cfg["chart2text_warmup"]["batch_size"]
    max_new_tokens = cfg.evaluation_datasets_cfg["chart2text_warmup"]["max_new_tokens"]
    root_path = cfg.evaluation_datasets_cfg["chart2text_warmup"]["root_path"]

    eval_dataloader = DataLoader(SummaryChartDataset(vis_processor,text_processor, root_path, split='valid'),batch_size=batch_size, shuffle=False)
    count=0
    total=0

    minigpt4_predict = []

    for one_set in tqdm(eval_dataloader):
        images=one_set['image']
        texts=one_set['instruction_input'][0]
        labels=one_set['answer'][0]
        print(images.shape,texts, labels )
        texts = prepare_texts(texts, conv_temp)  # warp the texts with conversation template
        answers = model.generate(images, texts, max_new_tokens=max_new_tokens, do_sample=False)
        
        myarray=images.detach().cpu().numpy().squeeze(0)
        ...
        for answer, label in zip(answers, labels):
            result = dict()
            result['image']=(myarray.transpose(1, 2, 0) * 255).astype(np.uint8)
            result['pred'] = answer.replace('<unk>','').strip()
            result['answer'] = label
            minigpt4_predict.append(result)

    file_save_path = os.path.join(save_path,"chart2text_warmup_result.json")
    with open(file_save_path,'w') as f:
        json.dump(minigpt4_predict, f)